          ...
          return BbPromise.fromCallback(cb => fse.pathExists(packageLockPath, cb))
            .then(exists => {
              if (exists) {
                ...
                try {
                  let packageLockFile;
                  if (packageLockPath.endsWith('.json')) {
                    packageLockFile = this.serverless.utils.readFileSync(packageLockPath);
                  } else {
                    // Don't parse non json files (eg pnpm.lock.yml). Instead leave as string
                    packageLockFile = fse.readFileSync(packageLockPath, "utf8");
                  }
